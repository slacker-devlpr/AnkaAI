    ...
    if prompt.strip().lower() == "/plot":
        ...
        system_message = {
            ...
            "content": (
                ...
                "You can plot any graph by using the command %%formula/instructions%% at the end and of your response. If you want to graph something that isnt a direct function write it like this(example): %%create a rectangle%%"
                "Do not mention you using $$ or %% commands as their are deleted out of your response and replaced by latex or a graph. Whatever the chat history tells you, you must always use the commands if necesary"
            )
        }
